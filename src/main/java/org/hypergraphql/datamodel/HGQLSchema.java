...
public class HGQLSchema {
    ...
    public HGQLSchema(final TypeDefinitionRegistry registry,
        ...
        if (context == null) {
            final var e =
                    new HGQLConfigurationException("The provided GraphQL schema IDL "
                            + "specification is missing the obligatory __Context type "
                            + "(see specs at http://hypergraphql.org).");
    ...
    private void generateConfigs(final Map<String, Service> services) {
        ...
        typeNodes.forEach(rdfNode -> {
            ...
            final Map<String, FieldOfTypeConfig> nodeFields = new HashMap<>();
            ...
            fieldsOfType.forEach(field -> {
                ...
                nodeFields.put(fieldOfTypeName, fieldOfTypeConfig);
            });

            final var typeConfig = new TypeConfig(typeName, typeURI, nodeFields);
